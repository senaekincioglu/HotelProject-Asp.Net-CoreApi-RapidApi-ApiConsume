...
namespace HotelProject.WebUI.ValidationRules.GuestValidationRules
{
    ...
    {
        public CreateGuestValidator() //ctor tab ile 
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("İsim alanı boş geçilemez");//Boş geçilemez
            RuleFor(x => x.Surname).NotEmpty().WithMessage("Soyisim alanı boş geçilemez");//Boş geçilemez
            RuleFor(x => x.City).NotEmpty().WithMessage("Şehir alanı boş geçilemez");//Boş geçilemez
