...
namespace RapidApiConsume.Controllers
{
    public class BookingByCityController : Controller
    {
        ...
        {
            if (!string.IsNullOrEmpty(cityID)) //Eğer boş değilse 
            {
                    ...
                    Headers =
    {
        ...
        { "X-RapidAPI-Host", "booking-com.p.rapidapi.com" },
                ...
                using (var response = await client.SendAsync(request))
                {
                    ...
                    var values = JsonConvert.DeserializeObject<BookingApiViewModel>(body);//Bu yazıldı.
                    return View(values.results.ToList());//Bu yazıldı

                }
            }
            else //aksi durumda Parisi getiricek.
            {
                var client = new HttpClient();
                var request = new HttpRequestMessage
                {
